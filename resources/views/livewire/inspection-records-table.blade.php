    ...
    <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-slate-50">
                <tr>
                    <th class="cursor-pointer px-4 py-3 text-left font-medium text-slate-600" wire:click="sortBy('licence_plate')">
                        {{ __('Plate') }}
                        @if ($sortField === 'licence_plate')
                            <span class="text-indigo-600">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
                    </th>
                    <th class="cursor-pointer px-4 py-3 text-left font-medium text-slate-600" wire:click="sortBy('customer_name')">
                        {{ __('Customer') }}
                    ...
                    <th class="px-4 py-3 text-left font-medium text-slate-600">{{ __('Category') }}</th>
                    <th class="cursor-pointer px-4 py-3 text-left font-medium text-slate-600" wire:click="sortBy('inspection_date')">
                        {{ __('Inspection') }}
                        @if ($sortField === 'inspection_date')
                            <span class="text-indigo-600">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
                    </th>
                    <th class="cursor-pointer px-4 py-3 text-left font-medium text-slate-600" wire:click="sortBy('expiration_date')">
                        {{ __('Expiry') }}
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($records as $record)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-mono text-xs font-semibold text-slate-800">{{ $record->licence_plate }}</td>
                        <td class="px-4 py-3 font-medium">{{ $record->customer_name }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $record->normalized_phone_number }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $record->vehicle_category ?? '—' }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $record->inspection_date?->format('d/m/Y') ?? '—' }}</td>
                        <td class="px-4 py-3">
                            @if ($record->expiration_date?->isPast())
                                <span class="font-medium text-red-600">{{ $record->expiration_date->format('d/m/Y') }}</span>
                            @elseif ($record->expiration_date?->diffInDays(now()) <= 7)
